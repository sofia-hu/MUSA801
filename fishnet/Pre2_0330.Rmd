...
x <- Train %>%  dplyr::select(-category, -interior, -vio_unsafe,
                               -vio_equip,-zoning,-isfire,-census_tract)
"rf_classifier = randomForest(fire ~ ., data=Train %>% 
                 dplyr::select(-category, -interior, -vio_unsafe,
                               -vio_equip,-zoning,-isfire,-census_tract) %>% 
                   sample_n(1000))"
...
preds <- data.frame(pred = predict(rf_classifier),
                    obs = Train$fire)
...
```{r}
ggplot(preds, aes(x = pred, fill = as.factor(obs))) + 
  geom_density() +
  facet_grid(obs ~ .) +
  scale_fill_manual(values = palette3) +
  labs(x = "Fire", y = "Density of probabilities",
...
testProbs <- 
  preds %>%
  mutate(predOutcome  = as.factor(ifelse(preds$pred > 0.12 , 1, 0)))%>%
  mutate(Outcome  = as.factor(preds$obs))
...
```{r}
ggplot(testProbs, aes(d = as.numeric(testProbs$Outcome), m = pred)) +
  geom_roc(n.cuts = 50, labels = FALSE, colour = "#FE9900") +
  style_roc(theme = theme_grey) +
  geom_abline(slope = 1, intercept = 0, size = 1.5, color = 'grey') +
  labs(title = "ROC Curve - logistic regression model")
