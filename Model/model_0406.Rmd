...
model_data$Med_Inc <-ifelse(is.na(model_data$Med_Inc.y),0,model_data$Med_Inc.y)
...
ggplot(testProbs, aes(d = as.numeric(testProbs$Outcome), m = Probs)) +
  ...
  labs(title = "ROC Curve - logistic regression model",subtitle = 'AUC = 0.6684')
...
ranger_model <- ranger(
    ...
    data=Train %>% 
                 dplyr::select(-category, -interior, -vio_unsafe,
                               -vio_equip,-zoning,-isfire), verbose = FALSE, importance = "impurity")
print(ranger_model)
...
DF<-as.data.frame(DF)
head(data.frame(ranger_model$variable.importance),15)
