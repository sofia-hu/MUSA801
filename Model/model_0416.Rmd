...
model_data <- subset(model_data, select = c(lagfire.nn5,lagfire20.nn5, lagfire19.nn5, lagfire18.nn5,
                                ...
                                had_intvio,issue_fixed,active_case))
...
random_nofire <- model_data_nofire[sample(nrow(model_data_nofire), nrow(model_data_havefire)*2), ]
...
testProbs <- 
  testProbs %>%
  mutate(predOutcome  = as.factor(ifelse(testProbs$Probs > 0.35 , 1, 0)))
...
```{r}
fire_fishnet$request19.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(request.sf)),3) 
fire_fishnet$light.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(request.sf2)),3) 
fire_fishnet$heat.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(request.sf3)),3) 
fire_fishnet$fire311.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(request.sf4)),3) 
fire_fishnet$infestation.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(request.sf5)),3) 
fire_fishnet$Detector.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(request.sf6)),3) 
fire_fishnet$Dangerous.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(request.sf7)),3) 
```

```{r}
fire_fishnet$lagfire15.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(fire.sf_152)),3)
fire_fishnet$lagfire16.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(fire.sf_162)),3)
fire_fishnet$lagfire17.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(fire.sf_172)),3)
fire_fishnet$lagfire18.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(fire.sf_182)),3)
fire_fishnet$lagfire19.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(fire.sf_192)),3)
fire_fishnet$lagfire20.nn3 <-nn_function(st_coordinates(st_centroid(fire_fishnet)),st_coordinates(st_centroid(fire.sf_202)),3)
```


```{r}
fire_fishnet <- 
  st_join(fire_fishnet,census1%>% st_transform(2272),st_covered_by,left=TRUE) 

fire_fishnet[is.na(fire_fishnet)] <- 0
```

```{r}
fire_fishnet <-
  st_join(fire_fishnet, dplyr::select(neighbor, NAME),st_nearest_feature,left=TRUE) %>%
...
## property level input (fishnet)

```{r}
fishnet_meanVal<-fire_fishnet %>% 
...
ranger_model <- ranger(
    ...
    data=Train %>% 
                 dplyr::select(-category, -interior.x, -vio_unsafe,
                               -vio_equip,-zoning,-isfire,-descript), verbose = FALSE, importance = "impurity")
print(ranger_model)
...
DF<-as.data.frame(DF)
head(data.frame(ranger_model$variable.importance),20)
...
confusionMatrix(as.factor(max.col(probabilities)-1), as.factor(Test$fire), positive = "1")
