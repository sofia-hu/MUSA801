...
### 2.2.1 How have fire events changed over time and what's the most common types?

The recordings of fire accidents has increased a lot in 2018. This may due to the change in the procedure for recording fires after 2017. in 2020, the number of fire events decreased. Considering the outbreak of the COVID 19, it seems reasonable. The plot on the right shows the most common type of building fire. Since we had sum up the fire events information to the parcel level, we left out the wild fire.
...
### 2.2.2 What's the spatial correlation of fire incidents?

The first risk factor is the spatial correlation of fire incidents. To understand if fires have a tendency to cluster in Philadelphia, we examined the average distance to 5 nearest fire event for each parcel. The result indicates that buildings with a smaller distance to past fires have a higher risk for future fire event.
...
When we plot the distance map of the fire events, we can't see that the yellow dots in the middle shows a clear clustering pattern.
...
```{r}
typeof(ensembleData.point$predOutcome)
```
...
ggplot() +
  geom_sf(data = neighborhoods, fill = "darkgrey") +
  geom_sf(data = ensembleData.point.above, 
          #aes(fill=Outcome), 
          colour = 'grey',size=0.02) +
  geom_sf(data = ensembleData.point.below, 
          #aes(fill=Outcome), 
          colour = 'coral',size=0.02) +
  labs(title= "Properties with Above-average Fire Risk") +
  #scale_fill_viridis(discrete = TRUE, direction = 1, option = "inferno",name="Sensitivity") +
  mapTheme()
```
...
ggplot() +
  geom_sf(data = neighborhoods, fill = "darkgrey") +
  geom_sf(data = ensembleData.point0, 
          #aes(fill=Outcome), 
          colour = 'grey',size=0.02) +
  geom_sf(data = ensembleData.point1, 
          #aes(fill=Outcome), 
          colour = 'coral',size=0.02) +
  labs(title= "Past Fires") +
  #scale_fill_viridis(discrete = TRUE, direction = 1, option = "inferno",name="Sensitivity") +
  mapTheme()
...
ggplot() +
  geom_sf(data = neighborhoods, fill = "darkgrey") +
  geom_sf(data = ensembleData.point0p, 
          #aes(fill=Outcome), 
          colour = 'grey',size=0.2) +
  geom_sf(data = ensembleData.point1p, 
          #aes(fill=Outcome), 
          colour = 'coral',size=0.2) +
  labs(title= "Predicted Fires") +
  #scale_fill_viridis(discrete = TRUE, direction = 1, option = "inferno",name="Sensitivity") +
  mapTheme()
```
...
```{r}
spatialCV_metrics <- spatialCV_metrics %>% 
    mutate(TRACT_NUM = as.numeric(census_tract))

spatialCV_forMapping <- left_join(tracts, spatialCV_metrics, 
                                   by = "TRACT_NUM") %>% 
  filter(!is.null(TRACT_NUM),
         !is.nan(TRACT_NUM))

ggplot() +
  geom_sf(data = tracts, fill = "darkgrey") +
  geom_sf(data = spatialCV_forMapping, 
          aes(fill=Sensitivity), colour = NA) +
  labs(title= "Accuracy of Ensemble Model \nacross Philadelphia Census Tracts",
       subtitle = "Sensitivity by Census Tract") +
  scale_fill_viridis(discrete = FALSE, direction = 1, option = "inferno",
                     name="Sensitivity") +
  mapTheme()
```
