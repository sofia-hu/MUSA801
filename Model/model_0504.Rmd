...
fire$lagfire.nn5 <-nn_function(st_coordinates(parcel.sf2),st_coordinates(fire.sf),5)
#write.table(fire,"fire0504.csv",row.names=FALSE,col.names=TRUE,sep=",")
...
fire_tracts_census <- 
  st_join(fire_tracts,census1%>% st_transform(2272),join = st_intersects,left=TRUE) 
...
```{r}
model_data2 <- subset(model_data, select = c(lagfire.nn5,lagfire20.nn5, lagfire19.nn5, lagfire18.nn5,
                                             lagfire17.nn5, lagfire16.nn5, lagfire15.nn5,
                                             category, interior.x,vio_unsafe, vio_equip,
                                             request19.nn5, light.nn5, heat.nn5, 
                                             fire311.nn5, infestation.nn5, Detector.nn5,                                             Dangerous.nn5,isfire,zoning,census_tract,Total_Pop,
                                            Med_Inc,Med_Age,Percent_White
                                            ,viol_count,related_violation,had_extvio,
                                had_intvio,violation.nn5))

model_data2 <- parcel.sf2 

model_data2 <- subset(model_data2, select = c(lagfire.nn5,lagfire20.nn5, lagfire19.nn5, lagfire18.nn5,
                                             lagfire17.nn5, lagfire16.nn5, lagfire15.nn5,
                                             category, interior.x,vio_unsafe, vio_equip,
                                             request19.nn5, light.nn5, heat.nn5, 
                                             fire311.nn5, infestation.nn5, Detector.nn5,                                             Dangerous.nn5,isfire,zoning,census_tract,Total_Pop,
...
model_data2$fire <- ifelse(model_data2$isfire=='Have fire',1,0)
table(model_data2$fire)

model_data2$isunsafe<-ifelse(is.na(model_data2$vio_unsafe),0,1)
model_data2$noequip<-ifelse(is.na(model_data2$vio_equip),0,1)
model_data2$iscom<-ifelse(model_data2$category=='Commercial',1,0)
model_data2$ishotel<-ifelse(model_data2$category=='Hotels and Apartments',1,0)
model_data2$isRM1<-ifelse(is.na(model_data2$zoning)|model_data2$zoning!='RM1',0,1)
model_data2$isCMX2<-ifelse(is.na(model_data2$zoning)|model_data2$zoning!='CMX2',0,1)
model_data2$issealed<-ifelse(is.na(model_data2$interior.x)|model_data2$interior.x!='Sealed / Structurally Compromised',0,1)
model_data2$isbelow<-ifelse(is.na(model_data2$interior.x)|model_data2$interior.x!='Below Average',0,1)

model_data2$Med_Inc <-ifelse(is.na(model_data2$Med_Inc),0,model_data2$Med_Inc)
model_data2$Med_Age<-ifelse(is.na(model_data2$Med_Age),0,model_data2$Med_Age)
model_data2$Percent_White<-ifelse(is.na(model_data2$Percent_White),0,model_data2$Percent_White)

#violation
model_data2$viol_count<-ifelse(is.na(model_data2$viol_count),0,model_data2$viol_count)
model_data2$related_violation<-ifelse(is.na(model_data2$related_violation),0,model_data2$related_violation)
model_data2$had_extvio <-ifelse(is.na(model_data2$had_extvio),0,model_data2$had_extvio)
model_data2$had_intvio<-ifelse(is.na(model_data2$had_intvio),0,model_data2$had_intvio)

####Down sampling
model_data2_havefire = model_data2 %>%
  filter(fire ==1)

#model_data2_havefire<- subset(model_data2_havefire, select = c(-descript))
#table(model_data2_havefire$fire)

model_data2_nofire= model_data2 %>% filter(fire == 0)
#table(model_data2_nofire$fire)

#select 108920 random rows from temp 0 (27230 *4=108920)
random_nofire <- model_data2_nofire[sample(nrow(model_data2_nofire), nrow(model_data2_havefire)*4), ]
#random_nofire<- subset(random_nofire, select = c(-descript))

#rbind 8900 rows of 0 to the 2225 rows of 1
model_data2_new <- rbind(model_data2_havefire,random_nofire)

#shuffle rows - generate a random ordering
set.seed(2021) ## make reproducible here, but not if generating many random samples
rand <- sample(nrow(model_data2_new))
model_data2_new<-model_data2_new[rand,]

model_data2_new2<-model_data2_new %>% st_set_geometry(NULL)
  
####Modeling
set.seed(3456)
trainIndex <- createDataPartition(model_data2_new2$fire, p = .75,
                                  list = FALSE,
                                  times = 1)
Train <- model_data2_new2[ trainIndex,]
Test  <- model_data2_new2[-trainIndex,]
...
pR2(log_reg)
 ...
 'to_pedict <- data.frame(lagfire.nn5 = lagfire.nn5, iscom = iscom, ishotel = ishotel, is_RM1 = isRM1, is_CMX2 = isCMX2, is_sealed = issealed, is_below = isbelow, is_unsafe = isunsafe, had_noequip = noequip, request19 = request19.nn5, light = light.nn5, heat = heat.nn5, fire311 = fire311.nn5, infestation = infestation.nn5, detector = Detector.nn5, dangerous = Dangerous.nn5, isfire = fire, census_tract_id = census_tract, pop = Total_Pop, income = Med_Inc, age = Med_Age, white = Percent_White, violation_count = viol_count,related_vio_count = related_violation, had_ex_violaiton = had_extvio, had_in_violation = had_intvio, violation = violation.nn5)'

##save model
pred_risk <- function(lagfire.nn5,iscom,ishotel,isRM1,isCMX2, issealed, isbelow, isunsafe, noequip,request19.nn5, light.nn5, heat.nn5,fire311.nn5, infestation.nn5, Detector.nn5, Dangerous.nn5, census_tract, Total_Pop, Med_Inc,Med_Age, Percent_White,viol_count,related_violation,had_extvio, had_intvio)
  {
  to_pedict <- data.frame(lagfire.nn5 = lagfire.nn5, iscom = iscom, ishotel = ishotel, isRM1 = isRM1, isCMX2 = isCMX2, issealed = issealed, isbelow = isbelow, isunsafe = isunsafe, noequip = noequip, request19.nn5 = request19.nn5, light.nn5 = light.nn5, heat.nn5 = heat.nn5, fire311.nn5 = fire311.nn5, infestation.nn5 = infestation.nn5, Detector.nn5 = Detector.nn5, Dangerous.nn5 = Dangerous.nn5, census_tract = census_tract, Total_Pop = Total_Pop, Med_Inc = Med_Inc, Med_Age = Med_Age, Percent_White = Percent_White, viol_count = viol_count, related_violation = related_violation, had_extvio = had_extvio, had_intvio = had_intvio)

  prob <-  predict(log_reg, to_pedict, type= "response")
  risk <- ifelse(prob>=avg,'Above average','Below average')
  risk
}

pred_risk(304.8299, 0, 0, 0, 0, 0, 0, 0, 0, 69.05813, 360.1478, 950.8937, 1653.84, 1543.537, 337.3111, 602.3736, 330, 3344, 28636, 28.5, 0.01854067, 0, 0, 0, 0)

write_rds(log_reg, "fire_risk_model_v2.rds")
```
...
cvFit
```
the model does not generalize as well with respect to specificity
...
ranger_model <- ranger(
    ...
    data=Train %>% 
                 dplyr::select(-category, -interior.x, -vio_unsafe,
                               -vio_equip,-zoning,-isfire,-census_tract,
                               -lagfire20.nn5, -lagfire19.nn5, -lagfire18.nn5,
                                             -lagfire17.nn5, -lagfire16.nn5, -lagfire15.nn5), verbose = FALSE, importance = "impurity")
print(ranger_model)
#importance_pvalues(rf.sim, method = "janitza")

pred_ranger <- predict(ranger_model, Test)$predictions
#pred_ranger <- predict(ranger_model, Test)

Range.ConMatx = confusionMatrix(pred_ranger, 
                                Test$fire, positive = "1")
Range.ConMatx

```

```{r,results=FALSE}
t <- data.frame(ranger_model$variable.importance)
importance<-as.numeric(ranger_model$variable.importance)
variable<-row.names(t)
DF<-cbind(variable,importance)
DF<-as.data.frame(DF)
head(data.frame(ranger_model$variable.importance),30)

```

#### ranger model(probability)
...
ggplot(testProbs_ranger, aes(d = as.numeric(testProbs_ranger$Outcome), m = Probs)) +
  ...
  style_roc(theme = theme_grey) +
  geom_abline(slope = 1, intercept = 0, size = 1.5, color = 'grey') +
  labs(title = "ROC Curve - ranger model",subtitle = 'AUC = 0.6747')

#auc(testProbs_ranger$Outcome, testProbs_ranger$Probs)
```
